...
class TestKCI:
    ...
    def test_custom_kernels(self, kci_data):
        ...
        test = KCI(data=df_vstruct, kernel_X=RBF(0.5), kernel_Y=RBF(0.5), kernel_Z=RBF(0.5))
        assert not test("X", "Y", ["Z"], significance_level=0.05)
    ...
    def test_large_sample_and_median_bandwidth(self):
        ...
        assert KCI(data=df)("X", "Y", ["Z"], significance_level=0.05)
        assert KCI(data=df, bandwidth="median")("X", "Y", ["Z"], significance_level=0.05)
